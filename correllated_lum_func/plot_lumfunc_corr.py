...
for band in 'gri':
    for distance in [(a,a+200) for a in range(100,1000,200)]:
        ...
        surf_bright = []
        V_max = []
        print "avg dist ",  2.0/3.0 * (distance[1]**3 - distance[0]**3)/(distance[1]**2 - distance[0]**2)
        vol_corr = 4.0*np.pi/3.0*(distance[1]**2 - distance[0]**2)**1.5 / 1.0e9 #in mpc
        vol_corr = vol_corr /(2.0/3.0 * (distance[1]**3 - distance[0]**3)/(distance[1]**2 - distance[0]**2)/5000.0)
        print "distance, vol_corr"
        print distance, vol_corr


        for choice in ['CMASS','CMASS_blanks_centered']:#, 'CMASS_blanks']:
            for typesearch in ['all','galaxy','star']:
                ztmp, vmaxtmp, petrotmp, hradtmp, abtmp, sbtmp  = get_alldat(band, distance, choice,typesearch = typesearch)
                z.append(ztmp)
                absmag.append(abtmp)
        ...
        plot_sample(z, absmag, petromag, halflight_rad, surf_bright, V_max, plot_stem = './plots/corr_lum_all_centered_%sband_%s_corr' %(band,distance[0]), colors = [colors['i'][0],colors['r'][0],colors['g'][0],colors['i'][1],colors['r'][1],colors['g'][1]], line_styles=['solid','solid','solid','solid','solid','solid'], title=title)

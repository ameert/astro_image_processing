...
data = get_data(cursor, '%s_lackner_%s' %(band, model), '%s_sdss_%s' %(band, tablestem), flags = use_flags, flagmodel = flagmodel, add_tables = ', %s_lackner_%s as z' %(band, model), conditions = ' and z.galcount = a.galcount ')
...
oplot.add_bars('r')
oplot.savefig('%s_%s_%s_%s_%s_lackner_lackner.eps' %(band, tablestem, model, xchoice, ychoice))

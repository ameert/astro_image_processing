...
filepath = '/data2/home/ameert/flagging/%s/%s' %(band,model)

for count in range(1,2684):
    print count
    infile = open('%s/total_profile_%d.pickle' %(filepath, count))
    new_data = open('%s/total_profile_%d_multitarget.pickle' %(filepath, count))
    ...
    os.system('rm %s/total_profile_%d_multitarget.pickle' %(filepath, count))

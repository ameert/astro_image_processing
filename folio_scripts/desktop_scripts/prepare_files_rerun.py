...
while chunk < np.ceil(float(galmax)/1000.0):
    ...
    for mod_count, model in enumerate(models):
        ...
        if not os.path.isdir('./fits/%s/%d' %(model, chunk+1)):
            os.system('mkdir ./fits/%s/%d' %(model, chunk+1))
...
rootname = 'r'
#Filter = 'r' 
outdir = '/data2/home/ameert/sdss_sample/fits_no_det/%s'  #the directory containing output data
...
""" %( "sdss_sex_"+str(chunk+1)+'.cat',"sdss_r_"+str(chunk+1)+'.cat',"sdss_r_out_" + str(chunk+1)+'.cat',model+'/'+str(chunk+1)+'/',good_rep[2][mod_count], good_rep[3][mod_count],good_rep[0][mod_count],good_rep[1][mod_count])


        config = open("./fits_no_det/%s/%d/config.py" %(model, chunk+1), "w")
        config.write(file)

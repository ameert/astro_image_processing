...
main_path = '/home/ameert/z_ICL_sims/sims/sim_image/'
exptime =53.907456
def insert_im(data_gal, back_dat, rowc, colc):
    ...
    backrange[0,:] = np.round(backrange[0,:]-inctr).astype(int)
    backrange[1,:] = np.round(backrange[1,:]+inctr).astype(int)
    ...
    for a in [0,1]:
        if back_lims[1,a]>shape[a]:
            imlims[1,a]=inshape[a]-(back_lims[1,a]-shape[a])-1
        else:
    ...
    backxdiff = back_lims[1,0]-back_lims[0,0]
    backydiff = back_lims[1,1]-back_lims[0,1]
    imxdiff = imlims[1,0]-imlims[0,0]
    imydiff = imlims[1,1]-imlims[0,1]
    
    if imydiff >backydiff:
      imlims[1,1] -= (imydiff-backydiff)
    elif imydiff <backydiff:
      back_lims[1,1] -= (backydiff-imydiff)
    
    if imxdiff >backxdiff:
      imlims[1,0] -= (imxdiff-backxdiff)
    elif imxdiff <backxdiff:
      back_lims[1,0] -= (backxdiff-imxdiff)
    ...
    outim =back_dat[:,:] 
    print "shapes"
    print outim[back_lims[0,0]:back_lims[1,0],back_lims[0,1]:back_lims[1,1]].shape
    print data_gal[imlims[0,0]:imlims[1,0],imlims[0,1]:imlims[1,1]].shape
...
def combine_head(header, back_head, rowc, colc, icl=False):
    ...
    for key in header.keys():
        if key in ['IE', 'ID','N_SER','BT','RD','RE','ED','EB','BANG','DANG']:
            ...
            #new_head.update(stem+key, header[key][0], header[key][1])
        else:
            new_head.update(key, header[key])#, header.comments[key])
            #new_head.update(key, header[key][0], header[key][1])
...
if __name__=="__main__":
    ...
    sim_to_choice =  np.array([1,8,15,22,29,36,43,50,57,64], dtype=int)
    choice = 0
    zchoice = 0

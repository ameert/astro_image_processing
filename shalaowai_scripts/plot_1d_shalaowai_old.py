...
def make_plot(data_dict, arcsec = True):          
    ...
    if arcsec:
        new_extent[0] = new_extent[0]*params.defaults['pixsz']
        new_extent[1] = new_extent[1]*params.defaults['pixsz']
...
"""
    ...
    for var_name in add_info_left:
        pl.figtext(0.02, sup_y, var_name, fontsize = 11,
    ...
    pl.figtext(0.58, 0.03, label_string, fontsize = 8,horizontalalignment = 'center')
    ...
    pl.ylim(28,np.min( np.array([ux, 18.0])))
    ...
    if yes_leg:
        ...
        pl.figlegend(handles, labels, loc = 3,bbox_to_anchor=(0.005, 0.03, 0.07, .15), title="1D Legend", prop = prop)

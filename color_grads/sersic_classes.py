...
for I_tot and Re"""
    ...
    def make_image(self):
        ...
        while counts_to_sim > 0:
            ...
            running_image, xedge, yedge = np.histogram2d(xcoords, ycoords, 
                       bins = [self.xsize, self.ysize], 
                       range = [[0, self.xsize],[0,self.ysize]]) 
...
class galaxy:
        ...
        def __init__(self, main_path, Name, Ie_counts, Id_counts, rd_arcsec, 
            ...
            if half_light > 0:
                self.x_size = 2.0*20.0*half_light
            else: 
                if self.rd > 300:
                    self.x_size = np.max([self.re,-1.0*self.rd]) *2* 20.0
                else:
                    self.x_size = np.max([self.re,self.rd]) * 2*20.0
        ...
        def make_profile(self, output_profile_filename = "NULL"):
            ...
            sersic_com = sersic(self.Ie_counts, self.ser, self.bang, self.ba, self.re)
            ...
            new_gal =  im_obj(sersic_com.image+disk_com.image)
            if do_convolve:
                new_gal.convolve_image(self.psf_name)
            else:
                new_gal.convolved_image = new_gal.image.astype(float)

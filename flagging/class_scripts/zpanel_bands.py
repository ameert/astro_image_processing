...
if sql_values['normtype'] == 'xbin':
    sql_values['savename']='./dist_obs_gri_{model}_{choice}.eps'.format(**sql_values)
elif sql_values['normtype'] == 'flagclass':
    sql_values['savename']='./dist_obs_class_per_gri_{model}_{choice}.eps'.format(**sql_values)
elif sql_values['normtype'] == 'total':
    sql_values['savename']='./dist_obs_total_per_gri_{model}_{choice}.eps'.format(**sql_values)
...
scount=-1 #for setting plot numbers correctly
for band in 'gri':
    ...
    if sql_values['choice']=='absmagrad':
        ...
        if sql_values['band']=='g':
            ax2.set_xlim((13.0,19.5))
            ax1.set_xlim((13.0,19.5))
        ...
        props['datamask'] = np.where(np.array(props['total'])>0,True,False)
        props = flag_norm(flags_to_use, props, sql_values['normtype'])
        ax1, ax2 = plot_props('r$_{{hl, {band}, tot, arcsec}}$'.format(**sql_values), props, radbins, delta, flags_to_use,plot_info)
        pl.xlim((0,8))
        ...
        props['datamask'] = np.where(np.array(props['total'])>0,True,False)
        props = flag_norm(flags_to_use, props, sql_values['normtype'])
        ax1, ax2 = plot_props('b/a$_{{tot, {band} }}$'.format(**sql_values), props, radbins, delta, flags_to_use,plot_info)
        pl.xlim((0,1))
        ...
        props['datamask'] = np.where(np.array(props['total'])>0,True,False)
        props = flag_norm(flags_to_use, props, sql_values['normtype'])
        ax1, ax2 = plot_props('n$_{{bulge, {band} }}$'.format(**sql_values), 
                              props, radbins, delta, flags_to_use,plot_info)
        pl.xlim((0,8))

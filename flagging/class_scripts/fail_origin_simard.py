...
def get_vals(binval1, binval2): 
    cmd = """select a.galcount, a.flag, %s, %s from Flags_catalog as a, M2010 as b, CAST as c, DERT as d, r_band_serexp as f, SSDR6 as z, simard.simard_serexp as x   where x.galcount = c.galcount and a.flag >=0 and a.band = 'r' and a.model = 'serexp' and a.ftype = 'u' and a.galcount = b.galcount and a.galcount = c.galcount and a.galcount = d.galcount and a.galcount = f.galcount and a.galcount = z.galcount and a.flag&pow(2,20)>0 order by a.galcount limit 1000000;""" %(binval1, binval2)

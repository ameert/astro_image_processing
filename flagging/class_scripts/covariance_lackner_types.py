...
cmd = """select d.galcount,c.flag, IF(d.model='dvc', 1,0)+IF(d.model='exp', 2,0)+IF(d.model='ser', 4,0)+IF(d.model='nb1', 8,0)+IF(d.model='nb4', 16,0) from catalog.Flags_optimize as c, catalog.r_lackner_fit as d where c.galcount = d.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by d.galcount;""".format(model = model, band = 'r')

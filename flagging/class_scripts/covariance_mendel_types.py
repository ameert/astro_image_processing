...
cmd = """select d.galcount,c.flag, IF(d.ProfType=1, 1,0)+IF(d.ProfType=2, 2,0)+IF(d.ProfType=4, 4,0)+IF(d.ProfType=3, 16,0) from catalog.Flags_optimize as c, simard.Mendel_masses as d where c.galcount = d.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by d.galcount;""".format(model = model, band = 'r')

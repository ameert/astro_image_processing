...
cmd = """select d.galcount,c.flag, IF(d.ProfType=1, 1,0)+IF(d.ProfType=2, 2,0)+IF(d.ProfType=4, 3,0)+IF(d.ProfType=3, 4,0) from catalog.Flags_optimize as c, catalog.r_simard_fit as d where c.galcount = d.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by d.galcount;""".format(model = model, band = 'r')

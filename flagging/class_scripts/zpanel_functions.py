...
def get_vals(binval, sql_values, cursor):
    if binval == 'lackner':
        cmd = """select d.galcount,IF(d.model='dvc',7,0)+ IF(d.model='ser' and z.n_bulge>=2.0, 12,0)+IF(d.model='exp',8,0)+ IF(d.model='ser' and z.n_bulge<2.0, 11,0)+IF(d.model='nb1',9,0)+IF(d.model = 'nb4', 10,0), {add_param} from catalog.M2010 as m, catalog.CAST as r,catalog.DERT as s, catalog.r_lackner_fit as d, catalog.r_lackner_ser as z, catalog.{band}_lackner_{model} as b where b.galcount = r.galcount and r.galcount=s.galcount and r.galcount = m.galcount and  m.galcount=d.galcount and d.galcount = z.galcount order by d.galcount limit {galnumlim};""".format(**sql_values)
        ...
        cmd = """select d.galcount,IF(d.model='dvc',7,0)+ IF(d.model='ser' and z.n_bulge>=2.0, 12,0)+IF(d.model='exp',8,0)+ IF(d.model='ser' and z.n_bulge<2.0, 11,0)+IF(d.model='nb1',9,0)+IF(d.model = 'nb4', 10,0),n.ttype,  {add_param} from   catalog.CAST as r,catalog.DERT as s,catalog.M2010 as m, catalog.r_lackner_fit as d, catalog.r_lackner_ser as z, catalog.Nair as n , catalog.{band}_lackner_{model} as b where b.galcount = r.galcount and  r.galcount=s.galcount and r.galcount = m.galcount and  m.galcount=d.galcount and n.galcount = d.galcount and d.galcount = z.galcount order by d.galcount  limit {galnumlim};""".format(**sql_values)
    elif binval == 'nair_simard':
        cmd = """select d.galcount, IF(d.Prob_pS>0.32 and z.n_bulge>=2.0, 12,0)+IF(d.Prob_pS>0.32 and z.n_bulge<2.0, 11,0)+IF(d.Prob_pS<=0.32 and d.Prob_n4>0.32, 13,0)+IF(d.Prob_pS<=0.32 and d.Prob_n4<=0.32, 14,0),  n.ttype,  {add_param} from   catalog.CAST as r,catalog.DERT as s,catalog.M2010 as m,  catalog.r_lackner_fit as c, catalog.r_simard_fit as d, catalog.r_simard_ser as z, catalog.Nair as n , catalog.{band}_simard_{model} as b where b.galcount = r.galcount and  r.galcount=s.galcount and r.galcount = m.galcount and  m.galcount = c.galcount and n.galcount = c.galcount  and c.galcount = d.galcount and c.galcount = z.galcount order by d.galcount   limit {galnumlim};""".format(**sql_values)
    elif binval == 'nair_meert':
        cmd = """select c.galcount,IF(c.flag&pow(2,1)>0,1,0)+ IF(c.flag&pow(2,4)>0,2,0)+ IF(c.flag&pow(2,10)>0 and d.n_bulge<7.95,3,0)+ IF(c.flag&pow(2,14)>0,4,0)+ IF(c.flag&pow(2,10)>0 and d.n_bulge>=7.95,6,0)+IF(c.flag&pow(2,20)>0,5,0)  ,n.ttype,   {add_param} from   catalog.CAST as r,catalog.DERT as s,catalog.M2010 as m, catalog.Flags_catalog as c, catalog.r_lackner_fit as z, catalog.r_band_serexp as d, catalog.Nair as n  , catalog.{band}_band_{model} as b where b.galcount = r.galcount and  r.galcount=s.galcount and r.galcount = m.galcount and m.galcount = c.galcount and n.galcount = c.galcount and d.galcount = c.galcount and c.galcount = z.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by c.galcount  limit {galnumlim};""".format(**sql_values)
    elif binval == 'galzoo':

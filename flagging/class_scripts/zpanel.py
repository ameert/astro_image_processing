...
def get_vals(binval): 
#    cmd = """select a.galcount, a.flag, {binval} from Flags_optimize as a, M2010 as b, CAST as c, DERT as d, {band}_band_{model} as f, SSDR6 as z   where a.flag >=0 and a.band = 'r' and a.model = '{model}' and a.ftype = 'u' and a.galcount = b.galcount and a.galcount = c.galcount and a.galcount = d.galcount and a.galcount = f.galcount and a.galcount = z.galcount order by a.galcount limit 1000000;""" .format(band=band, model=model, binval=binval)

    cmd = """select a.galcount, IF(a.flag&pow(2,10)>0, IF(f.n_bulge>7.95, a.flag^(pow(2,10)+pow(2,27)),a.flag),a.flag) , {binval} from Flags_optimize as a, M2010 as b, CAST as c, DERT as d, {band}_band_{model} as f where a.flag >=0 and a.band = 'r' and a.model = '{model}' and a.ftype = 'u' and a.galcount = b.galcount and a.galcount = c.galcount and a.galcount = d.galcount and a.galcount = f.galcount order by a.galcount limit 10000000;""".format(band=band, model=model, binval=binval)

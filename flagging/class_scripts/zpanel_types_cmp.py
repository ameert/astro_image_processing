...
def get_vals(binval): 
    if binval == 'lackner':
        cmd = """select d.galcount,c.flag, IF(d.model='dvc' or (d.model='ser' and z.n_bulge>=2.0), 1,0)+IF(d.model='exp' or (d.model='ser' and z.n_bulge<2.0), 2,0)+IF(d.model='nb1'or d.model = 'nb4', 3,0) from catalog.Flags_optimize as c, catalog.r_lackner_fit as d, catalog.r_lackner_ser as z where c.galcount = d.galcount  and c.galcount = z.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by d.galcount;""".format(model = 'serexp', band = 'r')
    elif binval == 'simard':
        cmd = """select d.galcount,c.flag, IF(d.Prob_pS>0.32 and z.n_bulge>=2.0, 1,0)+IF(d.Prob_pS>0.32 and z.n_bulge<2.0, 2,0)+IF(d.Prob_pS<=0.32, 3,0) from catalog.Flags_optimize as c, catalog.r_simard_fit as d, catalog.r_simard_ser as z where c.galcount = d.galcount and c.galcount = z.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by d.galcount;""".format(model = 'serexp', band = 'r')
    elif binval == 'mendel':
        cmd = """select d.galcount,c.flag, IF(d.ProfType=1 or (d.ProfType=4 and z.n_bulge >=2.0) , 1,0)+IF(d.ProfType=2 or (d.ProfType=4 and z.n_bulge <2.0), 2,0)+IF(d.ProfType=3, 3,0) from catalog.Flags_optimize as c, simard.Mendel_masses as d,  catalog.r_simard_ser as z where c.galcount = d.galcount and c.galcount = z.galcount  and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by d.galcount;""".format(model = 'serexp', band = 'r')
    elif binval == 'nair':
        cmd = """select d.galcount,c.flag, d.Ttype from catalog.Flags_optimize as c, catalog.Nair as d where c.galcount = d.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by d.galcount;""".format(model = 'serexp', band = 'r')
    elif binval == 'nair_lackner':
        cmd = """select d.galcount, IF(d.model='dvc' or (d.model='ser' and z.n_bulge>=2.0), 2,0)+IF(d.model='exp' or (d.model='ser' and z.n_bulge<2.0), 16,0)+IF(d.model='nb1'or d.model = 'nb4', pow(2,10),0),  c.Ttype from catalog.r_lackner_fit as d, catalog.r_lackner_ser as z, catalog.Nair as c  where c.galcount = d.galcount  and c.galcount = z.galcount  order by d.galcount;"""
    elif binval == 'nair_simard':
        cmd = """select d.galcount, IF(d.Prob_pS>0.32 and z.n_bulge>=2.0, 2,0)+IF(d.Prob_pS>0.32 and z.n_bulge<2.0, 16,0)+IF(d.Prob_pS<=0.32, pow(2,10),0),  c.Ttype  from  catalog.Nair as c, catalog.r_simard_fit as d, catalog.r_simard_ser as z where c.galcount = d.galcount and c.galcount = z.galcount order by d.galcount;"""
    elif binval == 'nair_mendel':
        cmd = """select d.galcount, IF(d.ProfType=1 or (d.ProfType=4 and z.n_bulge >=2.0) ,2,0)+IF(d.ProfType=2 or (d.ProfType=4 and z.n_bulge <2.0), 16,0)+IF(d.ProfType=3, pow(2,10),0) ,  c.Ttype from catalog.Nair as c, simard.Mendel_masses as d,  catalog.r_simard_ser as z where c.galcount = d.galcount and c.galcount = z.galcount order by d.galcount;"""
    elif binval == 'nair_lackner_meert':
        cmd = """select d.galcount, c.flag,  d.Ttype from catalog.Flags_optimize as c, catalog.r_lackner_fit as z, catalog.Nair as d  where c.galcount = d.galcount  and c.galcount = z.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by d.galcount;""".format(model = 'serexp', band = 'r')
    elif binval == 'meert':
        cmd = """select c.galcount,c.flag,  -6.0*m.probaEll -3.0*m.probaS0+4.0*m.probaSab+8.0*m.probaScd from catalog.Flags_optimize as c, catalog.M2010 as m where c.galcount = m.galcount and c.band='{band}' and c.model = '{model}' and c.ftype = 'u' order by c.galcount;""".format(model = 'serexp', band = 'r')

...
info_dict = {'dba':'catalog', 'usr':'pymorph', 'pwd':'pymorph', 'host':'',
             'band':'i', 'model':'serexp','autoflag_ftype':'r',
             'uflag_ftype':'u',}
...
"""& & Tiny Bulge, otherwise good &   %s \\\\ \\hline \\hline""",
...
def write_full_table_band(model, bands):
    row1str = ' c '*len(bands)
    row2str = ["""\\textbf{{\% {modname},\\emph{{{band}}}}}""".format(modname=model, band=band) for band in bands]
    ...
    for band in bands:
        indat1 = np.loadtxt('%s_%s_catalog.table' %(model, band), delimiter = ':', usecols = [1])
        indata.append(indat1)
...
"""\\end{tabular}"""]
    ...
    outfile = open('bands_{model}_table_tex.tex'.format(model=model), 'w')
    outfile.write( "\n".join(outstr))
    outfile.close()
    return
...
def model_table(flags, model_type):
    ...
    outfile = open('%s_%s_catalog.table' %(model_type,info_dict['band']), 'w')
